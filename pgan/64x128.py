...
class Model(GANModelDesc):
    ...
    def fromRGB(self, x, strval):
        x = myConv2D(strval, x, 256, kernel_shape=1, stride=1, nl=tf.identity)
        return x
    ...
    def generator(self, z, alpha):
        ...
        x_left = self.toRGB(x, 'toRGBtemp_64_128')
    ...
    def discriminator(self, x, alpha):
        ...
        # Alpha without convolution
        x_left = Downsample('disDowntemp_64_128', x)
        x_left = self.fromRGB(x_left, 'fromRGBtemp_64_128')
        x_left_alpha = tf.multiply((1 - alpha), x_left)

        x = self.fromRGBTrans(x, 'fromRGB_64_128')
    ...
    def _build_graph(self, inputs):
        ...
        real_img_prev = Upsample('real_img_prev_64_128', inputs[1])
