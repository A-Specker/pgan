...
class Model(GANModelDesc):
    ...
    def fromRGB(self, x, strval):
        x = myConv2D(strval, x, 128, kernel_shape=1, stride=1, nl=tf.identity)
        return x
        
    # Additional fromRGB method as inputs have different sized feature vectors
    def fromRGBTrans(self, x, strval):
        x = myConv2D(strval, x, 64, kernel_shape=1, stride=1, nl=tf.identity)
        return x
    ...
    def generator(self, z, alpha):
        ...
        x = Upsample('upsample004', x, 2)  # 64 64 512
        
        # Convolution layers for stage 64 64 256
        x = myConv2D('conv009', x, 256, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        x = myConv2D('conv010', x, 256, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        
        x = Upsample('upsample005', x, 2)  # 128 128 256
                
        # Convolution layers for stage 128 128 128
        x = myConv2D('conv011', x, 128, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        x = myConv2D('conv012', x, 128, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        
        x = Upsample('upsample006', x, 2)  # 256 256 128
        
        # Alpha without convolution
        x_left = self.toRGB(x, 'toRGBtemp_128_256')
        x_left_alpha = tf.multiply((1 - alpha), x_left)
        
        # Convolution layers for stage 256 256 64
        x = myConv2D('conv013', x, 64, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        x = myConv2D('conv014', x, 64, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = PixelwiseNorm(x)
        
        x = self.toRGB(x, 'toRGB_128_256')
    ...
    def discriminator(self, x, alpha):
        ...
        # Alpha without convolution
        x_left = Downsample('disDowntemp_128_256', x)
        x_left = self.fromRGB(x_left, 'fromRGBtemp_128_256')
        x_left_alpha = tf.multiply((1 - alpha), x_left)

        x = self.fromRGBTrans(x, 'fromRGB_128_256')
        
        # Convolution layers for stage 256 256 64/128
        x = Conv2D('conv014', x, 64, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = Conv2D('conv013', x, 128, kernel_shape=3, stride=1, nl=LeakyReLU)

        x = Downsample('downsample006', x) # 128 128 128
        
        # Alpha with convolution and subsequent combination
        x_right_alpha = tf.multiply(alpha, x)
        x = tf.add(x_left_alpha, x_right_alpha)
        
        # Convolution layers for stage 128 128 128/256
        x = myConv2D('conv012', x, 128, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = myConv2D('conv011', x, 256, kernel_shape=3, stride=1, nl=LeakyReLU)

        x = Downsample('downsample005', x) # 64 64 256
        
        # Convolution layers for stage 64 64 256/512
        x = myConv2D('conv010', x, 256, kernel_shape=3, stride=1, nl=LeakyReLU)
        x = myConv2D('conv009', x, 512, kernel_shape=3, stride=1, nl=LeakyReLU)

        x = Downsample('downsample004', x) # 32 32 512
        ...
        # Convolution layers for stage 4 4 512
        x = myConv2D('conv002', x, 512, kernel_shape=3, stride=1, nl=LeakyReLU) # 4 4 512
    ...
    def _build_graph(self, inputs):
        ...
        real_img_prev = Upsample('real_img_prev_128_256', inputs[1])
...
def get_data():
    ...
    ds = MapData(ds, lambda x: [resize(Image.fromarray(x[0]), H), resize(Image.fromarray(x[0]), H // 2)])
...
if __name__ == '__main__':
    ...
    GANTrainer(
        ...
        steps_per_epoch=15000, # <- 30.000 / BATCH_SIZE
